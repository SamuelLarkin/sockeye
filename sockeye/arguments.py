...
class ConfigArgumentParser(argparse.ArgumentParser):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.add_argument(
            "--config",
            help="Path to CLI arguments in yaml format "
    ...
    def _overwrite_add_argument(self, original_object):
        ...
        original_object.add_argument = types.MethodType(
            _new_add_argument, original_object
        )
    ...
    def check_greater_equal(value: str):
        ...
        if value_to_check < threshold:
            raise argparse.ArgumentTypeError(
                "must be greater or equal to %d." % threshold
            )
        return value_to_check
...
def float_greater_or_equal(threshold: float) -> Callable:
    ...
    def check_greater_equal(value: str):
        ...
        if value_to_check < threshold:
            raise argparse.ArgumentTypeError(
                "must be greater or equal to %f." % threshold
            )
        return value_to_check
...
def simple_dict() -> Callable:
    ...
    def parse(dict_str: str):
        ...
        try:
            ...
            raise argparse.ArgumentTypeError(
                "Specify argument dictionary as key1:value1,key2:value2,..."
            )
        return _dict
...
def multiple_values(
    ...
    def parse(value_to_check):
        if ":" in value_to_check:
            ...
            if expected_num_separators > 0 and (
                value_to_check.count(":") != expected_num_separators
            ):
                raise argparse.ArgumentTypeError(
                    "Expected either a single value or %d values separated by %s"
                    % (num_values, C.ARG_SEPARATOR)
                )
            values = tuple(
                map(data_type, value_to_check.split(C.ARG_SEPARATOR, num_values - 1))
            )
        else:
        ...
        if greater_or_equal is not None:
            if any((value < greater_or_equal for value in values)):
                raise argparse.ArgumentTypeError(
                    "Must provide value greater or equal to %d" % greater_or_equal
                )
        return values
...
def add_rerank_args(params):
    ...
    rerank_params.add_argument(
        "--output", "-o", default=None, help="File to write output to. Default: STDOUT."
    )
    rerank_params.add_argument(
        "--output-best",
        action="store_true",
...
def add_lexicon_create_args(params, is_for_block_lexicon: bool = False):
    ...
    if is_for_block_lexicon:
        ...
        input_help = (
            "Probabilistic lexicon (fast_align format) to build top-k lexicon from."
        )
    lexicon_params.add_argument("--input", "-i", required=True, help=input_help)
    lexicon_params.add_argument(
        "--output", "-o", required=True, help="File name to write top-k lexicon to."
    )
...
def add_lexicon_inspect_args(params):
    ...
    lexicon_params.add_argument(
        "--lexicon", "-l", required=True, help="File name of top-k lexicon to inspect."
    )
...
def add_logging_args(params):
    ...
    logging_params.add_argument(
        "--no-logfile", default=False, action="store_true", help="Suppress file logging"
    )
...
def add_device_args(params):
    ...
    device_params.add_argument(
        "--use-cpu", action="store_true", help="Use CPU device instead of GPU."
    )
    device_params.add_argument(
        "--env",
...
def add_model_parameters(params):
    ...
    model_params.add_argument(
        ...
        " Default: %(default)s.".format(values=", ".join(C.LHUC_CHOICES)),
    )
    ...
    model_params.add_argument(
        ...
        'Use "x:x" to specify separate values for src&tgt. Default: %d.'
        % C.DEFAULT_NUM_EMBED,
    )
    model_params.add_argument(
...
def add_batch_args(
    params, default_batch_size=4096, default_batch_type=C.BATCH_TYPE_WORD
):
    params.add_argument(
        "--batch-size",
        "-b",
...
def add_score_cli_args(params):
    ...
    add_device_args(params)
    add_batch_args(
        params, default_batch_size=56, default_batch_type=C.BATCH_TYPE_SENTENCE
    )

    params = params.add_argument_group("Scoring parameters")

    params.add_argument(
        "--model", "-m", required=True, help="Model directory containing trained model."
    )
...
def add_state_generation_args(params):
    ...
    params.add_argument(
        "--state-dtype",
        default=None,
        choices=[None, C.DTYPE_FP32, C.DTYPE_FP16],
...
def add_inference_args(params):
    ...
    decode_params.add_argument(
        ...
        "and %d * batch_size with batching."
...
def add_brevity_penalty_args(params):
    params.add_argument(
        ...
        type=str,
        choices=[
            C.BREVITY_PENALTY_NONE,
            C.BREVITY_PENALTY_LEARNED,
            C.BREVITY_PENALTY_CONSTANT,
        ],
...
def add_evaluate_args(params):
    ...
    eval_params.add_argument(
        "--references", "-r", required=True, type=str, help="File with references."
    )
    eval_params.add_argument(
        "--hypotheses",
        "-i",
