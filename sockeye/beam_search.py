...
class _SingleModelInference(_Inference):
    ...
    def encode_and_initialize(self, inputs: pt.Tensor, valid_length: pt.Tensor):
        states, predicted_output_length, nvs_prediction = (
            self._model.encode_and_initialize(inputs, valid_length, self._const_lr)
        )
        ...
        if target_factor_outputs:
            ...
            for i, tf_logits in enumerate(target_factor_outputs, 1):
                ...
                if target_prefix_factor_mask is not None:
                    tf_scores += target_prefix_factor_mask[:, :, i - 1].reshape(
                        -1, factor_vocab_size
                    )
            ...
            target_factors = (
                pt.cat(predictions, dim=1) if len(predictions) > 1 else predictions[0]
            )
...
class _EnsembleInference(_Inference):
    ...
    def encode_and_initialize(self, inputs: pt.Tensor, valid_length: pt.Tensor):
        ...
        for model in self._models:
            states, predicted_output_length, nvs_prediction = (
                model.encode_and_initialize(inputs, valid_length, self._const_lr)
            )
        ...
        predicted_output_lengths = (
            pt.stack(predicted_output_lengths, dim=1).float().mean(dim=1)
        )  # type: ignore
        nvs_prediction = (
    ...
    def decode_step(
        ...
        for model, model_state_structure in zip(self._models, self.state_structure()):
            model_states = states[
                state_index : state_index + len(model_state_structure)
            ]
            ...
            if target_factor_outputs:
                target_factor_probs = [
                    tfo.softmax(dim=-1) for tfo in target_factor_outputs
                ]
                if target_prefix_factor_mask is not None:
...
class SortNormalizeAndUpdateFinished(pt.nn.Module):
    ...
    def __init__(
        self, pad_id: int, eos_id: int, scorer: CandidateScorer, expect_factors: bool
    ) -> None:
        super().__init__()
        self.pad_id = pad_id
        self.eos_id = eos_id
        ...
        *factor_args,
        ...
        if self.expect_factors:
            ...
            updated_factor_scores = (
                factor_scores_accumulated.index_select(0, best_hyp_indices)
                + factor_scores
            )
...
class TopK(pt.nn.Module):
    ...
    def forward(self, scores):
        ...
        best_hyp_indices, best_word_indices = (
            indices.div(vocab_size, rounding_mode="floor"),
            indices.fmod(vocab_size),
        )
    ...
    def forward(self, *states):
        ...
        for state, state_format in zip(states, self.flat_structure):
            if state_format == C.STEP_STATE or state_format == C.MASK_STATE:
                ...
                raise ValueError(
                    "Provided state format %s not recognized." % state_format
                )
...
def _get_vocab_slice_ids(
    ...
    if vocab_slice_ids_size < beam_size + 1:
        ...
        vocab_slice_ids = pt.cat(
            (
                vocab_slice_ids,
                pt.full((n,), fill_value=eos_id, device=device, dtype=pt.int32),
            ),
            dim=0,
        )
...
def _get_nvs_vocab_slice_ids(
    ...
    if restrict_lexicon is not None:
        ...
        blocked_tokens = (
            pt.from_numpy(restrict_lexicon.get_blocked_trg_ids())
            .long()
            .to(nvs_prediction_above_thresh.device)
        )
    ...
    if len(bow) % 8 != 0:
        bow = pt.nn.functional.pad(
            bow, (0, 7 - ((len(bow) - 1) % 8)), mode="constant", value=C.EOS_ID
        )
...
class GreedySearch(Search):
    ...
    def forward(
        ...
        if target_prefix_factors is not None:
            target_prefix_factor_masks, target_prefix_factor_length = (
                utils.gen_prefix_masking(
        ...
        for t in range(1, max_iterations + 1):
            target_prefix_factor_mask = (
                ...
                if target_prefix_factor_masks is not None
                and t <= target_prefix_factor_length
                else None
            )
...
class BeamSearch(Search):
    ...
    def __init__(
        ...
        self._update_scores = UpdateScores(
            prevent_unk
        )  # tracing this module seems to incur a small slowdown on GPUs
    ...
    def forward(
        ...
        if target_prefix is not None:
            first_step_mask = utils.adjust_first_step_masking(
                target_prefix, first_step_mask
            )
        ...
        lengths = pt.zeros(
            batch_size * self.beam_size, device=self.device, dtype=pt.int32
        )
        finished = pt.zeros(
            batch_size * self.beam_size, device=self.device, dtype=pt.bool
        )
        ...
        scores_accumulated = pt.zeros(
            batch_size * self.beam_size, 1, device=self.device, dtype=self.dtype
        )
        ...
        # (0) encode source sentence, returns a list
        model_states, estimated_reference_lengths, nvs_prediction = (
            self._inference.encode_and_initialize(source, source_length)
        )
        ...
        for t in range(
            1, max_iterations + 1
        ):  # max_iterations + 1 required to get correct results
    ...
    def _should_stop(self, finished: pt.Tensor, batch_size: int) -> bool:
        if self.beam_search_stop == C.BEAM_SEARCH_STOP_FIRST:
            at_least_one_finished = (
                finished.reshape(batch_size, self.beam_size).sum(dim=1) > 0
            )
...
def get_search_algorithm(
    ...
    if greedy:
        ...
        assert constant_length_ratio == -1.0, (
            "Greedy search does not support brevity penalty"
        )
        assert sample is None, "Greedy search does not support sampling"
        assert not prevent_unk, (
            "Greedy Search does not support prevention of unknown tokens"
        )  # TODO: add support
        search = GreedySearch(
