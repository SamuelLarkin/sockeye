...
def get_fp32_state_dict_from_zero1_checkpoint(
    checkpoint_dir: str,
    ...
    with utils.smart_open(
        os.path.join(checkpoint_dir, C.DEEPSPEED_LATEST_NAME)
    ) as tag_in:
    ...
    state_dicts = [
        torch.load(fname, map_location=torch.device("cpu")) for fname in optim_files
    ]
    ...
    fp32_flat_groups = [
        state_dict[OPTIMIZER_STATE_DICT][SINGLE_PARTITION_OF_FP32_GROUPS]
...
def convert_checkpoint_to_params(
    model_config_fname: str, checkpoint_dirname: str, params_fname: str
):
    ...
    state_dict = {
        name[name.find(".") + 1 :]: param for (name, param) in state_dict.items()
    }
...
def convert_model_checkpoints(model_dirname: str, keep_deepspeed: bool = False):
    ...
    for fname in os.listdir(model_dirname):
        if (
            fname.startswith(C.PARAMS_PREFIX)
            and fname[len(C.PARAMS_PREFIX) :].isdigit()
        ):
            ...
            if os.path.isdir(params_fname):
                ...
                convert_checkpoint_to_params(
                    model_config_fname, checkpoint_dirname, params_fname
                )
