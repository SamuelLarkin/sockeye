...
def models_max_input_output_length(
    ...
    supported_max_seq_len_source = min(
        (model.max_supported_len_source for model in models)
    )
    supported_max_seq_len_target = min(
        (model.max_supported_len_target for model in models)
    )
...
class TranslatorInput:
    ...
    def get_source_prefix_tokens(self) -> Tokens:
        ...
        """
        return (
            self.source_prefix_tokens if self.source_prefix_tokens is not None else []
        )
    ...
    def get_target_prefix_tokens(self) -> Tokens:
        ...
        """
        return (
            self.target_prefix_tokens if self.target_prefix_tokens is not None else []
        )
    ...
    def with_eos(self) -> "TranslatorInput":
        ...
        return TranslatorInput(
            ...
            factors=[factor + [C.EOS_SYMBOL] for factor in self.factors]
            if self.factors is not None
            else None,
            source_prefix_tokens=self.source_prefix_tokens,
            source_prefix_factors=self.source_prefix_factors,
...
def _bad_input(sentence_id: SentenceId, reason: str = "") -> BadTranslatorInput:
    logger.warning(
        "Bad input (%s): '%s'. Will return empty output.", sentence_id, reason.strip()
    )
...
def make_input_from_plain_string(
    ...
    """
    return TranslatorInput(
        sentence_id, tokens=list(utils.get_tokens(string)), factors=None
    )
...
def make_input_from_dict(
    sentence_id: SentenceId, input_dict: Dict, translator: "Translator"
) -> TranslatorInput:
    """
    Returns a TranslatorInput object from a JSON object, serialized as a string.
    ...
    try:
        ...
        if source_prefix_factors is not None and not factors:
            ...
            return _bad_input(sentence_id, reason=str(input_dict))
        if source_prefix_tokens is not None and (
        ...
        if isinstance(source_prefix_factors, list):
            ...
            for source_prefix_factor in source_prefix_factors:
                if not source_prefix_factor:
                    logger.warning(
                        "Empty list is specified as source prefix factors for input '%s'.",
        ...
        use_target_prefix_all_chunks = input_dict.get(
        ...
        if (
            isinstance(translator.restrict_lexicon, dict)
        ...
        if isinstance(constraints, list):
            constraints = [
                list(utils.get_tokens(constraint)) for constraint in constraints
            ]
...
def _remove_target_prefix_tokens(
    target_ids: TokenIds, num_target_prefix_tokens: int
) -> TokenIds:
    """
...
class Translator:
    ...
    def translate(
        ...
        for trans_input_idx, trans_input in enumerate(trans_inputs):
            ...
            # empty input
                ...
                max_input_length_for_chunking = (
                    self.max_input_length - trans_input.num_source_prefix_tokens
                )
                    ...
                    input_chunks.extend(
                        [
                            IndexedTranslatorInput(
                                trans_input_idx, chunk_idx, chunk_input
                            )
        ...
        for batch_id, batch in enumerate(utils.grouper(input_chunks, batch_size)):
            ...
            translator_inputs = [
                indexed_translator_input.translator_input
                for indexed_translator_input in batch
            ]
            ...
            for chunk, translation in zip(batch, batch_translations):
                translated_chunks.append(
                    IndexedTranslation(chunk.input_idx, chunk.chunk_idx, translation)
                )
            num_batches += 1
    ...
    def _get_inference_input(
        ...
        target_prefix_factors_np = (
            np.zeros(
                (
                    batch_size,
                    max_target_prefix_factors_length,
        ...
        for j, trans_input in enumerate(trans_inputs):
            ...
            if (
                ...
                for i in range(1, self.num_target_factors):
                    target_prefix_factors_np[
                        j, : trans_input.num_target_prefix_factors, i - 1
                    ] = tokens2ids(
            ...
            if (
                not trans_input.source_prefix_factors
            ):  # no source prefix during inference
                ...
                for i, zip_of_factor_and_prefix_factor in enumerate(
                    zip(
                        ...
                        trans_input.source_prefix_factors[
                            : self.num_source_factors - 1
                        ],
                    ),
                    start=1,
                ):
    ...
    def _get_translation_tokens_and_factors(
        ...
        pruned_target_ids = (
            tokens for tokens in target_ids if tokens[0] not in self.strip_ids
        )
    ...
    def _assemble_translation(
        ...
        estimated_reference_length = (
            float(estimated_reference_length) if estimated_reference_length else None
        )
        return Translation(
            sequence,
            scores,  # type: ignore
...
def _unshift_target_factors(sequence: np.ndarray, fill_last_with: int = C.EOS_ID):
    ...
    sequence = [
        (w, *fs)
        for w, fs in itertools.zip_longest(_words, _next_factors, fillvalue=_fillvalue)
    ]  # type: ignore
    return sequence
