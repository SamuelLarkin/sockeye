...
def lexicon_iterator(
    path: str, vocab_source: Dict[str, int], vocab_target: Dict[str, int]
...
class TopKLexicon(RestrictLexicon):
    ...
    def __init__(
        self, vocab_source: Dict[str, int], vocab_target: Dict[str, int]
    ) -> None:
        ...
        self.always_allow = np.array(
            [vocab_target[symbol] for symbol in C.VOCAB_SYMBOLS], dtype="int32"
        )
    ...
    def create(self, path: str, k: int = 20):
        ...
        for src_id, group in groupby(
            lexicon_iterator(path, self.vocab_source, self.vocab_target),
            key=itemgetter(0),
        ):
            ...
            # filter trg_unk_id
            filtered_group = (
                (trg_id, prob) for src_id, trg_id, prob in group if trg_id != trg_unk_id
            )
            # sort by prob and take top k
            top_k = [
                trg_id
                for trg_id, prob in sorted(
                    filtered_group, key=itemgetter(1), reverse=True
                )[:k]
            ]
            if len(top_k) < k:
                num_insufficient += 1
        ...
        logger.info(
            'Created top-k lexicon from "%s", k=%d. %d source tokens with fewer than %d translations',
            path,
            k,
            num_insufficient,
            k,
    ...
    def load_np(self, lex: np.ndarray, k: Optional[int] = None):
        ...
        if k is not None:
            ...
            if k > loaded_k:
                logger.warning(
                    "Can not load top-%d translations from lexicon that "
    ...
    def get_allowed_trg_ids(self, src_ids: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        trg_ids = np.lib.arraysetops.union1d(
            self.always_allow, self.lex[unique_src_ids, :].reshape(-1)
        )  # type: ignore
        logger.debug(
            f"lookup: {trg_ids.shape[0]} unique targets for {unique_src_ids.shape[0]} unique sources"
        )
        return trg_ids
...
class StaticBlockLexicon(RestrictLexicon):
    ...
    def create(self, block_tokens: List[str], vocab_target: Dict[str, List[int]]):
        ...
        logger.info(
            "Created static block lexicon with %d tokens, %d skipped because they were not in the vocabulary",
            len(block_token_ids),
            num_not_in_vocab,
        )
...
def create_block_lexicon_from_file(args):
    ...
    with open(fname) as data:
        block_tokens = list(
            set(token for line in data for token in line.rstrip().split())
        )
        return create_block_lexicon_for_model(block_tokens, model_path, output_path)


def create_block_lexicon_for_model(
...
def inspect(args):
    ...
    for line in sys.stdin:
        ...
        print(
            "Input:  n=%d" % len(tokens),
            " ".join("%s(%d)" % (tok, i) for tok, i in zip(tokens, ids)),
        )
        ...
        print(
            "Output: n=%d" % len(tokens_trg),
            " ".join("%s(%d)" % (tok, i) for tok, i in zip(tokens_trg, trg_ids)),
        )
        print()
...
def main():
    ...
    params = argparse.ArgumentParser(
        description="Create or inspect a top-k lexicon for use during decoding."
    )
    subparams = params.add_subparsers(title="Commands")

    params_create = subparams.add_parser(
        "create",
        description="Create top-k lexicon for use during decoding. "
    ...
    params_block = subparams.add_parser(
        "create-block", description="Create block lexicon for use during decoding."
    )
    ...
    params_inspect = subparams.add_parser(
        "inspect", description="Inspect top-k lexicon for use during decoding."
    )
