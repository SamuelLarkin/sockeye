...
class LearningRateScheduler:
    ...
    def __call__(self, optimizer: torch.optim.Optimizer) -> "LearningRateScheduler":
        ...
        assert self.optimizer is None, (
            "This learning rate scheduler is already associated with an optimizer."
        )
        self.optimizer = optimizer
        return self
    ...
    def get_last_lr(self) -> List[float]:
        ...
        """
        assert self._last_lr is not None, (
            "`get_last_lr()` cannot be called before `get_lr()`"
        )
    ...
    def step(self, t: Optional[int] = None):
        ...
        assert self.optimizer is not None, (
            "This learning rate scheduler is not associated with an optimizer."
        )
        if t is None:
            t = self._t + 1
        self._t = t
...
class LearningRateSchedulerPlateauReduce(AdaptiveLearningRateScheduler):
    ...
    def __init__(
        ...
        self.lr = base_lr
        check_condition(
            0.0 < reduce_factor < 1, "reduce_factor should be between (0, 1)."
        )
    ...
    def new_evaluation_result(self, has_improved: bool) -> bool:
        ...
        if has_improved:
            ...
            if (
                ...
                logger.info(
                    "%d checkpoints since improvement or rate scaling, "
                    "lowering learning rate: %1.2e -> %1.2e",
                    self.num_not_improved,
                    old_lr,
                    self.lr,
                )
...
def get_lr_scheduler(
    ...
    if scheduler_type == C.LR_SCHEDULER_INV_SQRT_DECAY:
        return LearningRateSchedulerInvSqrtDecay, {
    ...
    if scheduler_type == C.LR_SCHEDULER_LINEAR_DECAY:
        ...
        return LearningRateSchedulerLinearDecay, {
            "base_lr": base_learning_rate,
    ...
    if scheduler_type == C.LR_SCHEDULER_PLATEAU_REDUCE:
        check_condition(
            ...
            "learning_rate_reduce_factor needed for %s scheduler"
            % C.LR_SCHEDULER_PLATEAU_REDUCE,
        )
        check_condition(
            learning_rate_reduce_num_not_improved is not None,
            "learning_rate_reduce_num_not_improved needed for %s scheduler"
            % C.LR_SCHEDULER_PLATEAU_REDUCE,
        )
