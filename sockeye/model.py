...
class SockeyeModel(pt.nn.Module):
    ...
    def __init__(
        ...
        self.embedding_source = encoder.Embedding(
            config.config_embed_source, embedding=source_embedding, dtype=self.dtype
        )
        self.embedding_target = encoder.Embedding(
            config.config_embed_target, embedding=target_embedding, dtype=self.dtype
        )
        ...
        self.factor_vocab_size = (
            factor_config.vocab_size if self.target_factor_configs else None
        )
        ...
        if mismatched_dtype_params:
            logger.warn(
                "Some parameters were created in a different dtype and then converted to the SockeyeModel's "
    ...
    def set_inference_only(self, inference_only: bool):
        ...
        self.output_layer = (
            self.output_layer_script_cached
            if self.inference_only
            else self.output_layer_module_cached
        )
    ...
    def encode(
        ...
        if self.traced_encoder is None:
            ...
            self.traced_encoder = pt.jit.trace(
                self.encoder, (source_embed, valid_length)
            )
        source_encoded, source_encoded_length, att_mask = self.traced_encoder(
    ...
    def encode_and_initialize(
        ...
        predicted_output_length = self.predict_output_length(
            source_encoded,
            source_encoded_lengths[:, 0],  # total source length
            constant_length_ratio,
        )
        # Decoder init states
        states = self.decoder.init_state_from_encoder(
            source_encoded, source_encoded_lengths
        )
        nvs_pred = None
        if self.nvs is not None:
            nvs_pred = pt.sigmoid(
                self.nvs(source_encoded, source_encoded_lengths, att_mask)
            )

        return states, predicted_output_length, nvs_pred

    def _embed_and_encode(
        self, source: pt.Tensor, source_length: pt.Tensor, target: pt.Tensor
        ...
        source_encoded, source_encoded_length, att_mask = self.encoder(
            source_embed, source_length
        )
        states = self.decoder.init_state_from_encoder(
            source_encoded, source_encoded_length, target_embed
        )
        nvs = None
        if self.nvs is not None:
    ...
    def decode_step(
        ...
        if self.traced_decode_step is None:
            ...
            self.traced_decode_step = pt.jit.trace(
                decode_step_module, decode_step_inputs
            )
        ...
        step_output, decoder_out, *target_factor_outputs = decode_step_outputs[
    ...
    def forward(self, source, source_length, target, target_length):  # pylint: disable=arguments-differ
        ...
        with (
            pt.no_grad()
            if self.train_decoder_only or self.forward_pass_cache_size > 0
            ...
            (
                ...
                states,
                nvs_prediction,
            ) = self.embed_and_encode(source, source_length, target)
        ...
        if self.length_ratio is not None:
            ...
            forward_output[C.LENRATIO_NAME] = self.length_ratio(
                source_encoded, source_encoded_length[:, 0]
            )
    ...
    def predict_output_length(
        ...
        if self.length_ratio is not None:
            # predicted_length_ratios: (batch_size,)
            predicted_length_ratio = self.length_ratio(
    ...
    def load_parameters(
        ...
        unexpected = [
            key for key in unexpected if "traced" not in key and "cached" not in key
        ]
        ...
        # to load.
        missing = [
            key for key in missing if "traced" not in key and "cached" not in key
        ]
        ...
        for module in self.modules():
            if hasattr(module, "kv_interleaved") and isinstance(
                module.kv_interleaved, bool
            ):
        ...
        if not allow_missing:
            for name, _ in model_params.items():
                assert name in new_params.keys(), (
                    "Parameter '%s' is missing in new_params dictionary. "
                    "Set allow_missing=True to ignore missing parameters." % name
                )
        for name in new_params:
            if not ignore_extra and name not in model_params:
                raise ValueError(
                    "Parameter '%s' in new_params dictionary is not present in ParameterDict. "
    ...
    def load_knn_index(self, knn_index_folder: str) -> None:
        ...
        )  # load returns a Config class, need to cast to subclass KNNConfig
    ...
    def _get_embeddings(
        self,
    ) -> Tuple[pt.nn.Embedding, pt.nn.Embedding, Optional[pt.nn.Parameter]]:
        """
        Returns embeddings for source, target, and output layer. Handles sharing and weight tying.
        """
        share_embed = (
            C.WEIGHT_TYING_SRC in self.config.weight_tying_type
        ...
        source_grad_sparse = (
            self.config.config_embed_source.allow_sparse_grad and not tie_weights
        )
        ...
        if share_embed:
            ...
            target_grad_sparse = (
                self.config.config_embed_target.allow_sparse_grad and not tie_weights
            )
...
def load_model(
    ...
    utils.check_condition(
        ...
        "number of source factors for model '%s' (%d)"
        % (len(source_vocabs), model_folder, model.num_source_factors),
    )
    utils.check_condition(
        model.num_target_factors == len(target_vocabs),
        "Number of loaded target vocabularies (%d) does not match "
        "number of target factors for model '%s' (%d)"
        % (len(target_vocabs), model_folder, model.num_target_factors),
    )
...
def load_models(
    ...
    for fi in range(len(first_model_vocabs)):
        utils.check_condition(
            vocab.are_identical(
                *[source_vocabs[i][fi] for i in range(len(source_vocabs))]
            ),
    ...
    for fi in range(len(first_model_vocabs)):
        utils.check_condition(
            vocab.are_identical(
                *[target_vocabs[i][fi] for i in range(len(target_vocabs))]
            ),
