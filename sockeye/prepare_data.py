...
def main():
    params = arguments.ConfigArgumentParser(
        description="Preprocesses and shards training data."
    )
...
def prepare_data(args: argparse.Namespace):
    ...
    num_shards = data_io.get_num_shards(
        num_sents, samples_per_shard, minimum_num_shards
    )
    logger.info(
        "%d samples will be split into %d shard(s) (requested samples/shard=%d, min_num_shards=%d)."
        % (num_sents, num_shards, samples_per_shard, minimum_num_shards)
    )
    shards, keep_tmp_shard_files = data_io.create_shards(
        source_fnames=source_paths,
