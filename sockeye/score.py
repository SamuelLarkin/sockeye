...
def main():
    params = arguments.ConfigArgumentParser(
        description="Score data with an existing model."
    )
...
def score(args: argparse.Namespace):
    ...
    if args.brevity_penalty_type == C.BREVITY_PENALTY_CONSTANT:
        if constant_length_ratio <= 0.0:
            ...
            logger.info(
                "Using constant length ratio saved in the model config: %f",
    ...
    scorer.score(
        ...
        output_handler=get_output_handler(
            output_type=args.output_type, output_fname=args.output
        ),
    )
