...
def generate_digits_file(
    ...
    with open(source_path, "w") as source_out, open(target_path, "w") as target_out:
        ...
        for _ in range(line_count - line_count_empty):
            digits = [
                random_gen.choice(_DIGITS)
                for _ in range(random_gen.randint(1, line_length))
            ]
...
def generate_json_input_file_with_tgt_prefix(
    ...
    tgt_factors_path: List[str] = None,
    seed=13,
):
    random_gen = random.Random(seed)
    with open(src_path, "r") as src_reader, open(tgt_path, "r") as tgt_reader:
        with open(json_file_with_tgt_prefix_path, "w") as out:
            ...
            if src_factors_path is not None:
                list_src_factors = [
                    open(src_factors, "r") for src_factors in src_factors_path
                ]
                list_src_factors = [
                    [sf.strip() for sf in src_factors]
                    for src_factors in list_src_factors
                ]

            if tgt_factors_path is not None:
                list_tgt_factors = [
                    open(tgt_factors, "r") for tgt_factors in tgt_factors_path
                ]
                list_tgt_factors = [
            ...
            for i, stdigits in enumerate(zip(src_reader, tgt_reader)):
                ...
                if len(tgt_digits) > 0:
                    random_pos = random_gen.choice(
                        [pos for pos in range(len(tgt_prefix))]
                    )
                ...
                if tgt_factors_path is not None and len(list_tgt_factors[0][i]) > 0:
                    ...
                    random_pos = random_gen.choice(
                        [pos for pos in range(len(list_tgt_factors[0][i]))]
                    )
...
def tmp_digits_dataset(
    ...
    with TemporaryDirectory(prefix=prefix) as work_dir:
        ...
        test_source_with_target_prefix_path = os.path.join(
            work_dir, "test_source_with_target_prefix.json"
        )
        ...
        source_factors_path = (
            None if "test_source_factors" not in data else data["test_source_factors"]
        )
        target_factors_path = (
            None if "test_target_factors" not in data else data["test_target_factors"]
        )
...
TRAIN_PARAMS_COMMON = (
    "--use-cpu --max-seq-len {max_len} --source {train_source} --target {train_target}"
    " --validation-source {dev_source} --validation-target {dev_target} --output {model}"
    " --seed {seed}"
)

PREPARE_DATA_COMMON = (
    " --max-seq-len {max_len} --source {train_source} --target {train_target}"
...
TRAIN_PARAMS_PREPARED_DATA_COMMON = (
    "--use-cpu --max-seq-len {max_len} --prepared-data {prepared_data}"
...
def run_train_translate(
    ...
    params = "{} {}".format(
        ...
        LEXICON_CREATE_PARAMS_COMMON.format(
            input=ttable_path, model=data["model"], topk=20, lexicon=lexicon_path
        ),
    )
    with patch.object(sys, "argv", params.split()):
    ...
    data["test_with_target_prefix_output"] = os.path.join(
        work_dir, "test_with_target_prefix.out"
    )
    ...
    data["test_with_target_prefix_outputs"] = collect_translate_output_and_scores(
        data["test_with_target_prefix_output"]
    )
    ...
    params = "{} {} {}".format(
        sockeye.translate.__file__,
        TRANSLATE_PARAMS_COMMON.format(
            model=data["model"], input=data["test_source"], output=data["test_output"]
        ),
        translate_params,
    )
    ...
    assert (
        len(data["test_inputs"])
        == len(data["test_targets"])
...
def run_translate_restrict(
    data: Dict[str, Any], translate_params: str
) -> Dict[str, Any]:
    """
    ...
    out_with_target_prefix_path = os.path.join(
        data["work_dir"], "out-with-target-prefix-restrict.txt"
    )
    ...
    params = "{} {} {} {}".format(
        translate_mod.__file__,
        TRANSLATE_PARAMS_COMMON.format(
            model=data["model"], input=data["test_source"], output=out_path
        ),
        translate_params,
        TRANSLATE_PARAMS_RESTRICT.format(lexicon=data["lexicon"], topk=1),
    )
    ...
    assert (
        len(data["test_with_target_prefix_outputs_restricted"])
        == len(data["test_outputs_restricted"])
        == len(data["test_outputs"])
    )
    return data
