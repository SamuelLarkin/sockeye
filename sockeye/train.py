...
def check_arg_compatibility(args: argparse.Namespace):
    ...
    if len(args.source_factors_share_embedding) > 1:
        ...
        args.source_factors_share_embedding = (
            args.source_factors_share_embedding * n_source_factors
        )
    ...
    if len(args.target_factors_share_embedding) > 1:
        ...
        args.target_factors_share_embedding = (
            args.target_factors_share_embedding * n_target_factors
        )
    ...
    if args.dtype != C.DTYPE_FP32:
        logger.warning(
            "Specifying a non-float32 dtype to sockeye.train has no effect. For 16-bit or mixed precision "
    ...
    if args.local_rank is not None:
        check_condition(
            ...
            "DeepSpeed mode does not support --amp or --apex-amp. "
            "Use --deepspeed-fp16 or --deepspeed-bf16.",
        )
        check_condition(
            not (
                args.learning_rate_scheduler_type == C.LR_SCHEDULER_PLATEAU_REDUCE
...
def check_resume(args: argparse.Namespace, output_folder: str) -> bool:
    ...
    if os.path.exists(output_folder):
        if args.overwrite_output:
            ...
            old_args = vars(
                arguments.load_args(os.path.join(output_folder, C.ARGS_STATE_NAME))
            )
            arg_diffs = _dict_difference(vars(args), old_args) | _dict_difference(
                old_args, vars(args)
            )
    ...
    if utils.is_distributed():
        if utils.is_primary_worker():
            os.makedirs(
                os.path.join(output_folder, C.DIST_SECONDARY_WORKERS_LOGDIR),
                exist_ok=True,
            )
...
def use_shared_vocab(args: argparse.Namespace) -> bool:
    ...
    if (
        C.WEIGHT_TYING_SRC in weight_tying_type
        and C.WEIGHT_TYING_TRG in weight_tying_type
    ):
        if not shared_vocab:
            logger.info(
                "A shared source/target vocabulary will be used as weight tying source/target weight tying "
                "is enabled"
            )
        shared_vocab = True
    return shared_vocab
...
def create_data_iters_and_vocabs(
    ...
    if utils.is_distributed():
        error_msg = (
            "Distributed training requires prepared training data. Use `python -m sockeye.prepare_data` and "
    ...
    if args.prepared_data is not None:
        ...
        train_iter, validation_iter, data_config, source_vocabs, target_vocabs = (
        ...
        if args.end_of_prepending_tag is not None:
            logger.warning(
                "The end-of-prepending tag specified in the prepared data will be used."
            )
...
def create_encoder_config(
    ...
    for factor_combine, factor_size in zip(
        args.source_factors_combine, args.source_factors_num_embed
    ):
    ...
    if total_source_factor_size > 0:
        logger.info(
            ...
            % (
                encoder_transformer_model_size,
                num_embed_source + total_source_factor_size,
            )
        )
    ...
    config_encoder = transformer.TransformerConfig(
        ...
        use_lhuc=args.lhuc is not None
        and (C.LHUC_ENCODER in args.lhuc or C.LHUC_ALL in args.lhuc),
...
def create_decoder_config(
    ...
    for factor_combine, factor_size in zip(
        args.target_factors_combine, args.target_factors_num_embed
    ):
        if factor_combine == C.FACTORS_COMBINE_CONCAT:
    ...
    if total_target_factor_size > 0:
        logger.info(
            ...
            % (
                decoder_transformer_model_size,
                num_embed_target + total_target_factor_size,
            )
        )
    ...
    config_decoder = transformer.TransformerConfig(
        ...
        max_seq_len_target=max_seq_len_target,
        use_lhuc=args.lhuc is not None
        and (C.LHUC_DECODER in args.lhuc or C.LHUC_ALL in args.lhuc),
...
def get_num_embed(args: argparse.Namespace) -> Tuple[int, int]:
    ...
    if not num_embed_source:
        logger.info(
            "Source embedding size was not set it will automatically be adjusted to match the "
            "Transformer source model size (%d).",
    ...
    for factor_combine, factor_size in zip(
        ...
        if factor_combine == C.FACTORS_COMBINE_CONCAT:
            total_source_factor_size += factor_size
    if total_source_factor_size > 0:
        adjusted_transformer_encoder_model_size = (
            num_embed_source + total_source_factor_size
        )
        check_condition(
            adjusted_transformer_encoder_model_size % 2 == 0
            and adjusted_transformer_encoder_model_size
            % args.transformer_attention_heads[0]
            == 0,
    ...
    if not num_embed_target:
        logger.info(
            "Target embedding size was not set it will automatically be adjusted to match the "
            "Transformer target model size (%d).",
    ...
    if total_target_factor_size > 0:
        adjusted_transformer_decoder_model_size = (
            num_embed_target + total_target_factor_size
        )
        check_condition(
            adjusted_transformer_decoder_model_size % 2 == 0
            and adjusted_transformer_decoder_model_size
            % args.transformer_attention_heads[0]
            == 0,
...
def create_model_config(
    ...
    config_encoder, encoder_num_hidden = create_encoder_config(
        args, max_seq_len_source, max_seq_len_target, num_embed_source
    )
    ...
    if len(source_vocab_sizes) > 1:
        ...
        if not source_factors_num_embed:
            ...
            source_factors_num_embed = [num_embed_source] * len(
    ...
    if len(target_vocab_sizes) > 1:
        ...
        if not target_factors_num_embed:
            ...
            target_factors_num_embed = [num_embed_target] * len(
    ...
    if args.length_task is not None:
        config_length_task = layers.LengthRatioConfig(
            num_layers=args.length_task_layers, weight=args.length_task_weight
        )
...
def create_losses(
    args: argparse.Namespace, all_num_classes: List[int]
) -> List[loss.Loss]:
    ...
    for i, (num_classes, weight) in enumerate(zip(all_num_classes, loss_weights)):
        ...
        label_smoothing = (
            args.label_smoothing if i == 0 else 0.0
        )  # Note: No label smoothing for target factor losses.
...
def create_optimizer_config(args: argparse.Namespace) -> optimizers.OptimizerConfig:
    ...
    num_workers = (
        1 if not utils.is_distributed() else torch.distributed.get_world_size()
    )
    effective_batch_size = args.batch_size * args.update_interval * num_workers
    logger.info(config)
    logger.info(
        f"Gradient accumulation over {args.update_interval} batch(es) by {num_workers} worker(s). Effective "
...
def create_deepspeed_config(
    ...
    utils.check_condition(
        utils.using_deepspeed(), "Initialize DeepSpeed before generating the config"
    )
...
def unset_requires_grad_for_fixed_params(
    ...
    if config.lhuc:
        ...
        fixed_param_names += [
            name for name in params if not name.endswith("lhuc.weight")
        ]
        logger.info("LHUC enabled, fixing all non-LHUC parameters")
    elif fixed_param_strategy is not None:
        fixed_param_names += fixed_param_names_from_strategy(
            config, params, fixed_param_strategy
        )
...
def fixed_param_names_from_strategy(
    ...
    def is_fixed(name: str) -> bool:
        ...
        if strategy == C.FIXED_PARAM_STRATEGY_ALL_EXCEPT_OUTER_LAYERS:
            ...
            return not (
                name.startswith(first_encoder_prefix)
                or name.startswith(last_encoder_prefix)
                or name.startswith(first_decoder_prefix)
                or name.startswith(last_decoder_prefix)
            )
        if strategy == C.FIXED_PARAM_STRATEGY_ALL_EXCEPT_EMBEDDINGS:
            # Any type of learned embedding.
            return not (
                name.startswith(C.SOURCE_EMBEDDING_PREFIX)
                or name.startswith(C.TARGET_EMBEDDING_PREFIX)
            )
        if strategy == C.FIXED_PARAM_STRATEGY_ALL_EXCEPT_OUTPUT_PROJ:
            # Target output projection.
            return not name.startswith(
                C.DEFAULT_OUTPUT_LAYER_PREFIX
            ) and not name.startswith(C.NVS_LAYER_PREFIX)
        if strategy == C.FIXED_PARAM_STRATEGY_ALL_EXCEPT_FEED_FORWARD:
            return not (
                name.endswith("ff.ff1.bias")
                or name.endswith("ff.ff1.weight")
                or name.endswith("ff.ff2.bias")
                or name.endswith("ff.ff2.weight")
            )
        if strategy == C.FIXED_PARAM_STRATEGY_ENCODER_AND_SOURCE_EMBEDDINGS:
            return name.startswith(C.ENCODER_PREFIX) or name.startswith(
                C.SOURCE_EMBEDDING_PREFIX
            )
...
def main():
    params = arguments.ConfigArgumentParser(
        description="Train Sockeye sequence-to-sequence models."
    )
...
def train(
    ...
    if not utils.is_primary_worker():
        logfile = os.path.join(
            output_folder,
            C.DIST_SECONDARY_WORKERS_LOGDIR,
    ...
    train_iter, eval_iter, config_data, source_vocabs, target_vocabs = (
    ...
    if max_seq_len_source != config_data.max_seq_len_source:
        logger.info(
            ...
            config_data.max_seq_len_source,
            max_seq_len_source,
        )
    ...
    if max_seq_len_target != config_data.max_seq_len_target:
        logger.info(
            ...
            config_data.max_seq_len_target,
            max_seq_len_target,
        )
    ...
    sockeye_model = model.SockeyeModel(
        ...
        train_decoder_only=args.fixed_param_strategy
    ...
    ds_config = (
        create_deepspeed_config(
            args, optimizer_config, optimizer_class, optimizer_kwargs
        )
        if utils.using_deepspeed()
        else None
    )
    ...
    if args.apex_amp:
        ...
        training_model, optimizer = apex.amp.initialize(
            training_model, optimizer, opt_level="O2"
        )
    ...
    if utils.is_primary_worker():
        checkpoint_decoder = create_checkpoint_decoder(
            args, device, sockeye_model, source_vocabs, target_vocabs
        )
    ...
    if utils.using_deepspeed() and utils.is_primary_worker():
        ...
        convert_deepspeed.convert_model_checkpoints(
            trainer_config.output_dir, keep_deepspeed=False
        )
